...
<div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100">
        ...
        <h3 class="text-lg font-bold text-slate-800">Formulir Edit Kartu Keluarga: {{ $kartuKeluarga->nomor_kk }}</h3>
    </div>
    ...
    <form action="{{ route('kartu-keluarga.update', $kartuKeluarga->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="nomor_kk" value="{{ old('nomor_kk', $kartuKeluarga->nomor_kk) }}" required maxlength="16" minlength="16"
        ...
        <div>
            ...
            <input type="text" name="kepala_keluarga" value="{{ old('kepala_keluarga', $kartuKeluarga->kepala_keluarga) }}" required
                   class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="rt" value="{{ old('rt', $kartuKeluarga->rt) }}" required maxlength="3"
                       class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                       placeholder="Contoh: 001">
            </div>
            <div>
                <label class="block text-xs uppercase font-bold text-slate-500 mb-2">RW <span class="text-red-500">*</span></label>
                <input type="text" name="rw" value="{{ old('rw', $kartuKeluarga->rw) }}" required maxlength="3"
                       class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                       placeholder="Contoh: 001">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="alamat" required rows="3"
                      class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                      placeholder="Masukkan alamat KK lengkap">{{ old('alamat', $kartuKeluarga->alamat) }}</textarea>
        </div>
