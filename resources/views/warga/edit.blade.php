...
<div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 max-w-4xl mx-auto">
    <div class="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100">
        ...
        <h3 class="text-lg font-bold text-slate-800">Formulir Edit Data Warga: {{ $warga->nama_lengkap }}</h3>
    </div>
    ...
    <form action="{{ route('warga.update', $warga->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nik" value="{{ old('nik', $warga->nik) }}" required maxlength="16" minlength="16"
                ...
                <div>
                    ...
                    <input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $warga->nama_lengkap) }}" required
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $warga->tanggal_lahir ? $warga->tanggal_lahir->format('Y-m-d') : '') }}" required
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition">
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-4 mt-2">
                        <label class="inline-flex items-center">
                            <input type="radio" name="jenis_kelamin" value="Laki-laki" {{ old('jenis_kelamin', $warga->jenis_kelamin) == 'Laki-laki' ? 'checked' : '' }} required class="w-4 h-4 text-emerald-600 border-slate-300 focus:ring-emerald-500">
                            <span class="ml-2 text-sm text-slate-700">Laki-laki</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="jenis_kelamin" value="Perempuan" {{ old('jenis_kelamin', $warga->jenis_kelamin) == 'Perempuan' ? 'checked' : '' }} required class="w-4 h-4 text-emerald-600 border-slate-300 focus:ring-emerald-500">
                ...
                <div>
                    ...
                    <select name="hubungan_keluarga" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        ...
                        @foreach(['Kepala Keluarga', 'Istri', 'Anak', 'Orang Tua', 'Mertua', 'Menantu', 'Cucu', 'Famili Lain'] as $hub)
                            <option value="{{ $hub }}" {{ old('hubungan_keluarga', $warga->hubungan_keluarga) == $hub ? 'selected' : '' }}>{{ $hub }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="pendidikan" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        ...
                        @foreach(['Tidak/Belum Sekolah', 'Belum Tamat SD', 'Tamat SD', 'SLTP/SMP', 'SLTA/SMA', 'Diploma I/II', 'Diploma III', 'Diploma IV/S1', 'S2', 'S3'] as $pend)
                            <option value="{{ $pend }}" {{ old('pendidikan', $warga->pendidikan) == $pend ? 'selected' : '' }}>{{ $pend }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="pekerjaan" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        ...
                        @foreach(['Belum/Tidak Bekerja', 'Mengurus Rumah Tangga', 'Pelajar/Mahasiswa', 'PNS', 'TNI', 'POLRI', 'Karyawan Swasta', 'Buruh Harian Lepas', 'Petani / Pekebun', 'Wiraswasta', 'Lainnya'] as $pek)
                            <option value="{{ $pek }}" {{ old('pekerjaan', $warga->pekerjaan) == $pek ? 'selected' : '' }}>{{ $pek }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status_perkawinan" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        ...
                        @foreach(['Belum Kawin', 'Kawin', 'Cerai Hidup', 'Cerai Mati'] as $stat)
                            <option value="{{ $stat }}" {{ old('status_perkawinan', $warga->status_perkawinan) == $stat ? 'selected' : '' }}>{{ $stat }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="nomor_telepon" value="{{ old('nomor_telepon', $warga->nomor_telepon) }}"
                           class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                ...
                <div>
                    ...
                    <textarea name="alamat" required rows="3"
                              class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
                              placeholder="Masukkan alamat lengkap warga saat ini">{{ old('alamat', $warga->alamat) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div>
            <h4 class="text-xs uppercase font-extrabold text-emerald-700 tracking-wider mb-4 border-b border-slate-100 pb-2">
                <i class="fa-solid fa-file-arrow-up mr-1.5"></i> Ganti Dokumen Pendukung (Maks 2MB)
            </h4>
                ...
                <div class="border border-dashed border-slate-200 p-4 rounded-xl text-center">
                    ...
                    @if($warga->foto)
                        <div class="mb-3 flex justify-center">
                            <img src="{{ asset('storage/' . $warga->foto) }}" class="w-16 h-16 rounded-full object-cover">
                        </div>
                    @endif
                ...
                <div class="border border-dashed border-slate-200 p-4 rounded-xl text-center">
                    ...
                    @if($warga->berkas_ktp)
                        <div class="mb-3 text-xs text-slate-500">
                            <span class="text-emerald-700 font-semibold"><i class="fa-solid fa-file-pdf"></i> Terunggah</span>
                        </div>
                    @endif
